...
procedure TfrmOnThisDay.FormShow(Sender: TObject);
 ...
 begin
  if otdate='0000' then begin
    ...
    wbOTD.Navigate(RunFrom+'data\'+UserSettings.LanguageSymbol+'\onthisday\'+s+'.htm');
  end
  else
    wbOTD.Navigate(RunFrom+'data\'+UserSettings.LanguageSymbol+'\onthisday\'+otdate+'.htm');
end;
...
procedure TfrmOnThisDay.cbShowAtStartupClick(Sender: TObject);
 begin
  UserSettings.ShowOnThisDay:=cbShowAtStartup.Checked;
end;

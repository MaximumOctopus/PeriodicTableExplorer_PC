...
<xsl:template match="map">
	<HTML><HEAD><TITLE>Country Index</TITLE>
	<style type="text/css">
	<xsl:comment>
	...
	<xsl:for-each select="country[@type='astop']">
		...
		<xsl:if test="@type='entry'">
			<xsl:choose>
				<xsl:when test="position() mod 2 = 1">
					...
					<DIV class="JF">&nbsp;&nbsp;<img src="bluecube.gif"/><a href="m{@code}.htm"><xsl:value-of select="title"/></a></DIV></td></tr>
				</xsl:when>		
				<xsl:otherwise>
					<tr><td>
					<DIV class="JF">&nbsp;&nbsp;<img src="bluecube.gif"/><a href="m{@code}.htm"><xsl:value-of select="title"/></a></DIV></td></tr>

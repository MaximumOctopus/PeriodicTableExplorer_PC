...
<xsl:template match="element">
	...
	<xsl:document href="{concat('compounds\c',$boing, '.htm')}">
		<HTML><HEAD>
...
<xsl:template match="compound">
	<TABLE width="100%">
		<TR><TD class="H1"><A name="{position()}" /><img src="bluecube.gif"/><xsl:value-of select="name" />&nbsp;<xsl:apply-templates select="formula" /></TD></TR>
